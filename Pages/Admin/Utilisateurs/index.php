...
$totalActifs = $stmt->fetch()['total'] ?? 0;

// Notifications
$notification = new Notification();
$totalNotifs = $notification->countNonLuesUtilisateur($idUtilisateur);

$page_title = 'Gestion des utilisateurs - SNEL Admin';
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<div class="main-content">
    ...
    <header class="topbar">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-lg font-bold text-primary hidden sm:block">Gestion des utilisateurs</h1>
                    <p class="text-xs text-gray-400 hidden sm:block">Agents et administrateurs</p>
                </div>
            </div>
            
            <div class="flex items-center space-x-4">
                <a href="ajouter.php" class="btn-success px-4 py-2 rounded-lg font-semibold text-sm flex items-center">
    ...
    <div class="p-4 lg:p-6">
        ...
        <div class="grid grid-cols-4 gap-4 mb-6">
            ...
            <div class="card-snel text-center border-l-4 border-indigo-500">
                <p class="text-xs text-gray-500">Agents</p>
                <p class="text-2xl font-bold text-indigo-600"><?= number_format($totalAgents) ?></p>
            </div>
            <div class="card-snel text-center border-l-4 border-green-500">
                <p class="text-xs text-gray-500">Actifs</p>
                <p class="text-2xl font-bold text-green-600"><?= number_format($totalActifs) ?></p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 mb-6">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="flex flex-wrap gap-2">
                    ...
                    <a href="?filtre=administrateur" class="filter-btn <?= $filtre === 'administrateur' ? 'active' : '' ?>">Admins</a>
                    <a href="?filtre=agent" class="filter-btn <?= $filtre === 'agent' ? 'active' : '' ?>">Agents</a>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <?php foreach ($utilisateurs as $utilisateurItem): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3">
                                        <p class="font-medium text-primary"><?= htmlspecialchars($utilisateurItem['prenom'] . ' ' . $utilisateurItem['nom']) ?></p>
                                        <p class="text-xs text-gray-400">ID: #<?= $utilisateurItem['idUtilisateur'] ?></p>
                                    </td>
                                    <td class="px-4 py-3 text-sm text-gray-600">
                                        <?= htmlspecialchars($utilisateurItem['email']) ?>
                                    </td>
                                    <td class="px-4 py-3">
                                        <span class="status-badge <?= $utilisateurItem['role'] === 'administrateur' ? 'status-admin' : 'status-agent' ?>">
                                            <i class="fas <?= $utilisateurItem['role'] === 'administrateur' ? 'fa-shield-alt' : 'fa-user-tie' ?> mr-1"></i>
                                            <?= ucfirst($utilisateurItem['role']) ?>
                                        </span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <span class="status-badge <?= $utilisateurItem['statut'] === 'actif' ? 'status-actif' : 'status-inactif' ?>">
                                            <?= ucfirst($utilisateurItem['statut']) ?>
                                        </span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <a href="details.php?id=<?= $utilisateurItem['idUtilisateur'] ?>" 
                                           class="btn-primary text-xs px-3 py-1 rounded-lg inline-flex items-center">
                                            <i class="fas fa-eye mr-1"></i> Voir
                                        </a>
                                        <a href="modifier.php?id=<?= $utilisateurItem['idUtilisateur'] ?>" 
        ...
        <?php else: ?>
            <div class="bg-white rounded-xl shadow-sm p-12 text-center">
                <div class="text-6xl text-gray-300 mb-4"><i class="fas fa-users-cog"></i></div>
                <h3 class="text-xl font-semibold text-gray-700 mb-2">Aucun utilisateur trouvé</h3>
                <p class="text-gray-500"><?= $search ? 'Aucun utilisateur ne correspond à votre recherche.' : 'Aucun utilisateur enregistré.' ?></p>
                <a href="ajouter.php" class="btn-secondary inline-block mt-4 px-6 py-2 rounded-lg">
