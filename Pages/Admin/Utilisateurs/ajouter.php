...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        try {
            ...
            $stmt = $db->prepare("SELECT idUtilisateur FROM Utilisateurs WHERE email = ?");
            $stmt->execute([$email]);
            if ($stmt->fetch()) {
                ...
                if ($result['success']) {
                    ...
                    $errors = $result['errors'] ?? ['database' => $result['error'] ?? 'Erreur lors de la création.'];
                }
            }
        } catch (Exception $e) {
            $errors[] = $e->getMessage();
...
$page_title = 'Ajouter un utilisateur - SNEL Admin';
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <header class="topbar">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                ...
                <h1 class="text-lg font-bold text-primary hidden sm:block">Ajouter un utilisateur</h1>
            </div>
            ...
            <div class="card-snel">
                ...
                <form method="POST" action="">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="email" name="email" class="input-field" placeholder="email@exemple.com" required>
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-semibold text-gray-700 mb-2">Téléphone</label>
                        <input type="tel" name="telephone" class="input-field" placeholder="+243 XX XXX XXXX">
                    </div>
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div class="mb-4">
                            ...
                            <input type="password" name="mot_de_passe" class="input-field" placeholder="Minimum 6 caractères" required>
                        </div>
                        <div class="mb-4">
                            <label class="block text-sm font-semibold text-gray-700 mb-2">Confirmer <span class="text-red-500">*</span></label>
                            <input type="password" name="confirm_password" class="input-field" placeholder="Confirmez le mot de passe" required>
                        </div>
                    </div>
