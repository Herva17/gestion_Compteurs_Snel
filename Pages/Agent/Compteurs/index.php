<?php
// Pages/Agent/Compteurs/index.php
session_start();
...
$stmt = $db->query("SELECT COUNT(*) as total FROM Compteur WHERE etat = 'inactif'");
...
$page_title = 'Compteurs - SNEL Agent';
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<aside class="sidebar" id="sidebar">
    <div class="p-5">
        ...
        <nav class="space-y-1">
            ...
            <a href="../Compteurs/index.php" class="nav-item active">
        ...
        <div class="flex items-center justify-between">
            ...
            <div class="flex items-center space-x-4">
                <a href="nouveau.php" class="btn-success px-4 py-2 rounded-lg font-semibold text-sm flex items-center">
                    <i class="fas fa-plus mr-2"></i> Nouveau compteur
                </a>
                <div class="relative">
    ...
    <div class="p-4 lg:p-6">
        ...
        <div class="grid grid-cols-4 gap-4 mb-6">
            ...
            <div class="card-snel text-center border-l-4 border-yellow-500">
                ...
                <p class="text-2xl font-bold text-yellow-600"><?= number_format($inactifs) ?></p>
            </div>
        </div>
                            ...
                            <?php foreach ($compteurs as $compteur): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-4 py-3 text-sm font-medium text-primary">
                                        <?= htmlspecialchars($compteur['NumeroSerie'] ?? 'N/A') ?>
                                        <p class="text-xs text-gray-400">Inst. le <?= isset($compteur['DateInstallation']) ? date('d/m/Y', strtotime($compteur['DateInstallation'])) : 'Non installé' ?></p>
                                    </td>
                                    ...
                                    <td class="px-4 py-3 text-sm font-bold text-primary">
                                        <?= number_format($compteur['indexActuel'] ?? 0, 0, ',', ' ') ?> kWh
                                    </td>
                                    ...
                                    <td class="px-4 py-3">
                                        <span class="status-badge <?= ($compteur['etat'] ?? '') === 'actif' ? 'status-actif' : (($compteur['etat'] ?? '') === 'en_panne' ? 'status-panne' : 'status-inactif') ?>">
                                            <?= ucfirst($compteur['etat'] ?? 'Inconnu') ?>
                                        </span>
                                    </td>
                                    <td class="px-4 py-3">
                                        <a href="details.php?id=<?= $compteur['idCompteur'] ?? 0 ?>" 
        ...
        <?php else: ?>
            <div class="bg-white rounded-xl shadow-sm p-12 text-center">
                ...
                <a href="nouveau.php" class="btn-secondary inline-block mt-4 px-6 py-2 rounded-lg">
                    <i class="fas fa-plus mr-2"></i> Ajouter un compteur
                </a>
            </div>
        <?php endif; ?>
