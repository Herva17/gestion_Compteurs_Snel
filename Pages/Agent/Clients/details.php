<?php
// Pages/Agent/Clients/details.php
session_start();
...
if ($ids) {
    ...
    $stmt = $db->prepare("SELECT c.*, cp.numeroSerie FROM Consommation c JOIN Compteur cp ON c.idCompteur = cp.idCompteur WHERE c.idCompteur IN ($placeholders) ORDER BY c.dateFin DESC LIMIT 15");
    $stmt->execute($ids);
...
$stmt = $db->prepare("SELECT * FROM Paiement WHERE idClient = ? ORDER BY datePaiement DESC");
$stmt->execute([$idClient]);
...
$page_title = 'Fiche client - SNEL Agent';
?>
<!DOCTYPE html>
<html lang="fr">
<head>
...
<aside class="sidebar" id="sidebar">
    <div class="p-5">
        ...
        <nav class="space-y-1">
            ...
            <a href="/Pages/Agent/Clients/index.php" class="nav-item active">
    ...
    <div class="p-4 lg:p-6">
        <div class="max-w-6xl mx-auto">
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <?php if ($totalImpaye > 0): ?>
                            <span class="bg-red-50 text-red-700 px-3 py-1 rounded-full text-sm flex items-center">
                                <i class="fas fa-exclamation-triangle mr-1"></i> <?= number_format($totalImpaye, 0, ',', ' ') ?> FCFA impayé
                            </span>
                        <?php endif; ?>
                    </div>
                </div>
            ...
            <div class="grid md:grid-cols-2 gap-4 mb-6">
                <div class="card-snel">
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between"><span class="text-gray-500">Email</span><span class="font-medium"><?= htmlspecialchars($client['email']) ?></span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Téléphone</span><span class="font-medium"><?= htmlspecialchars($client['telephone']) ?></span></div>
                        <?php if ($client['telephone2']): ?>
                            <div class="flex justify-between"><span class="text-gray-500">Téléphone 2</span><span class="font-medium"><?= htmlspecialchars($client['telephone2']) ?></span></div>
                        <?php endif; ?>
                        <div class="flex justify-between"><span class="text-gray-500">Adresse</span><span class="font-medium"><?= htmlspecialchars($client['adresse'] ?: 'Non renseignée') ?></span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Ville</span><span class="font-medium"><?= htmlspecialchars($client['ville'] ?: 'Non renseignée') ?></span></div>
                    </div>
                </div>
                ...
                <div class="card-snel">
                    ...
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between"><span class="text-gray-500">Factures totales</span><span class="font-medium"><?= $totalFactures ?></span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Factures impayées</span><span class="font-medium text-red-500"><?= count($facturesImpayees) ?></span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Montant impayé</span><span class="font-bold text-secondary"><?= number_format($totalImpaye, 0, ',', ' ') ?> FCFA</span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Total payé</span><span class="font-medium text-green-600"><?= number_format($totalPaye, 0, ',', ' ') ?> FCFA</span></div>
                        <div class="flex justify-between"><span class="text-gray-500">Consommation totale</span><span class="font-medium"><?= number_format($totalConso, 2, ',', ' ') ?> kWh</span></div>
                    </div>
                </div>
            </div>
                            ...
                            <div class="bg-gray-50 rounded-lg p-4 border border-gray-200">
                                ...
                                <div class="mt-3 flex gap-2">
                                    <a href="/Pages/Agent/Compteurs/details.php?id=<?= $compteur['idCompteur'] ?>" 
                                       class="btn-primary text-xs px-3 py-1 rounded-lg flex-1 text-center">
                                        <i class="fas fa-eye mr-1"></i> Voir
                                    </a>
                                    <a href="/Pages/Agent/Releves/nouveau.php?compteur=<?= $compteur['idCompteur'] ?>" 
                                       class="btn-secondary text-xs px-3 py-1 rounded-lg flex-1 text-center">
                ...
                <div class="overflow-x-auto">
                    <table class="w-full">
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <?php foreach (array_slice($consommations, 0, 10) as $conso): ?>
            ...
            <div class="flex flex-wrap justify-center gap-4 mt-6">
                ...
                <a href="/Pages/Agent/Releves/nouveau.php?client=<?= $idClient ?>" class="btn-secondary px-6 py-2 rounded-lg">
                    <i class="fas fa-edit mr-2"></i> Relever
                </a>
                <a href="modifier.php?id=<?= $idClient ?>" class="btn-primary px-6 py-2 rounded-lg">
                    <i class="fas fa-pen mr-2"></i> Modifier
                </a>
                <a href="/Pages/Agent/Compteurs/nouveau.php?client=<?= $idClient ?>" class="btn-primary px-6 py-2 rounded-lg">
                    <i class="fas fa-plus mr-2"></i> Ajouter compteur
                </a>
            </div>
