<?php
// Pages/Agent/Paiements/index.php
session_start();
...
if ($filtre === 'effectue') {
    $where[] = "p.statut = 'effectue'";
} elseif ($filtre === 'en_attente') {
    $where[] = "p.statut = 'en_attente'";
}

if (!empty($search)) {
    $where[] = "(p.numeroReference LIKE ? OR cl.nom LIKE ? OR cl.prenom LIKE ? OR f.numeroFacture LIKE ?)";
    $searchTerm = "%$search%";
    $params[] = $searchTerm;
    $params[] = $searchTerm;
    $params[] = $searchTerm;
...
$sql .= " ORDER BY p.datePaiement DESC LIMIT 50";
...
// Total paiements
$stmt = $db->query("SELECT COUNT(*) as total FROM Paiement");
$totalPaiements = $stmt->fetch()['total'] ?? 0;

// Total collecté
$stmt = $db->query("SELECT SUM(montant) as total FROM Paiement WHERE statut = 'effectue'");
$totalMontant = $stmt->fetch()['total'] ?? 0;

// Paiements du mois - CORRECTION : utilisation de prepare() au lieu de query()
$stmt = $db->prepare("SELECT COUNT(*) as total FROM Paiement WHERE MONTH(datePaiement) = ? AND YEAR(datePaiement) = ? AND statut = 'effectue'");
$stmt->execute([date('m'), date('Y')]);
$paiementsMois = $stmt->fetch()['total'] ?? 0;

// Paiements en attente
$stmt = $db->query("SELECT COUNT(*) as total FROM Paiement WHERE statut = 'en_attente'");
$paiementsAttente = $stmt->fetch()['total'] ?? 0;

// Notifications
$notification = new Notification();
$totalNotifs = $notification->countNonLuesUtilisateur($idUtilisateur);

$page_title = 'Paiements - SNEL Agent';
?>
<!DOCTYPE html>
<html lang="fr">
<head>
    ...
    <style>
        * { font-family: 'Inter', sans-serif; }
        ...
        .status-effectue { background: #dcfce7; color: #166534; }
    ...
    <div class="p-5">
        ...
        <nav class="space-y-1">
            ...
            <a href="index.php" class="nav-item active">
                ...
                <span class="badge"><?= $totalPaiements ?></span>
            </a>
    ...
    <header class="topbar">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                ...
                <div>
                    <h1 class="text-lg font-bold text-primary hidden sm:block">Paiements</h1>
                    <p class="text-xs text-gray-400 hidden sm:block">Historique des paiements</p>
                </div>
            </div>
            
            <div class="flex items-center space-x-4">
                <a href="nouveau.php" class="btn-success px-4 py-2 rounded-lg font-semibold text-sm flex items-center">
    ...
    <div class="p-4 lg:p-6">
        ...
        <div class="grid grid-cols-4 gap-4 mb-6">
            <div class="card-snel text-center">
                <p class="text-xs text-gray-500">Total paiements</p>
                <p class="text-2xl font-bold text-primary"><?= number_format($totalPaiements) ?></p>
            </div>
            <div class="card-snel text-center border-l-4 border-secondary">
                <p class="text-xs text-gray-500">Total collecté</p>
                <p class="text-2xl font-bold text-secondary"><?= number_format($totalMontant, 0, ',', ' ') ?> F</p>
            </div>
            <div class="card-snel text-center border-l-4 border-green-500">
                <p class="text-xs text-gray-500">Ce mois</p>
                <p class="text-2xl font-bold text-green-600"><?= number_format($paiementsMois) ?></p>
            </div>
            <div class="card-snel text-center border-l-4 border-yellow-500">
                <p class="text-xs text-gray-500">En attente</p>
                <p class="text-2xl font-bold text-yellow-600"><?= number_format($paiementsAttente) ?></p>
            </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm p-4 mb-6">
            <div class="flex flex-wrap items-center justify-between gap-4">
                <div class="flex flex-wrap gap-2">
                    ...
                    <a href="?filtre=effectue" class="filter-btn <?= $filtre === 'effectue' ? 'active' : '' ?>">Effectués</a>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            <?php foreach ($paiements as $paiement): ?>
                                <tr class="hover:bg-gray-50">
                                    ...
                                    <td class="px-4 py-3">
                                        <span class="status-badge <?= $paiement['statut'] === 'effectue' ? 'status-effectue' : 'status-attente' ?>">
        ...
        <?php else: ?>
            <div class="bg-white rounded-xl shadow-sm p-12 text-center">
                ...
                <p class="text-gray-500"><?= $search ? 'Aucun paiement ne correspond à votre recherche.' : 'Aucun paiement enregistré.' ?></p>
